...
1.  Asegúrate de haber activado el entorno virtual (`source venv/bin/activate`).
2.  Exporta las variables de entorno si no usas un sistema que las cargue automáticamente.
    ```bash
...
1.  Crea un directorio de configuración para el bot:

    ```bash
    sudo mkdir -p /etc/telegram-bot
    ```

2.  Crea el fichero de entorno con tus secretos:

    ```bash
    sudo nano /etc/telegram-bot/bot.env
    ```

3.  Añade el siguiente contenido, **sin comillas**:
...
2.  Pega el siguiente contenido. **Presta especial atención a `User`, `WorkingDirectory` y `ExecStart`** y modifícalos según tu configuración.
...
1.  **Recargar `systemd`** para que detecte el nuevo servicio:
    ```bash
    sudo systemctl daemon-reload
    ```
2.  **Habilitar el servicio** para que se inicie al arrancar el sistema:
    ```bash
...
3.  **Iniciar el servicio** ahora mismo:
    ...
    ```
4.  **Comprobar el estado** para ver si está corriendo y si hay errores:
    ```bash
...
1.  Para añadir un nuevo idioma o actualizar traducciones, edita los ficheros `.po` en `locales/<idioma>/LC_MESSAGES/`.
2.  Después de editar, compila el fichero `.po` a formato `.mo`:
    ```bash
...
Edit and rename`configbot.json.example` to `configbot.json`. Define paths to your scripts, the services you want to manage, servers to monitor, etc. Secrets are no longer stored here.
...
2.  Export the environment variables if you are not using a system that loads them automatically.
    ```bash
...
1.  Create a configuration directory for the bot:
...
3.  Add the following content, **without quotes**:
...
2.  Paste the following content. **Pay close attention to `User`, `WorkingDirectory`, and `ExecStart`** and modify them according to your setup.
...
3.  **Start the service** now:
    ...
    sudo systemctl start telegram-bot.service
    ```
4.  **Check the status** to see if it's running and to check for errors:
    ```bash
    sudo systemctl status telegram-bot.service
    ```
...
1.  To add a new language or update translations, edit the `.po` files in `locales/<language_code>/LC_MESSAGES/`.
2.  After editing, compile the `.po` file into the `.mo` format:
    ```bash
    msgfmt -o locales/es/LC_MESSAGES/base.mo locales/es/LC_MESSAGES/base.po
    ```
