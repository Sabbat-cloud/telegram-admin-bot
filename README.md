...
## 🌟 Main Features
  ...
  * **🔌 Service Management:** Start, stop, restart, and check the status of system services (`systemd`) and Docker containers.
  * **🖥️ Command Execution:** Securely execute terminal commands.
  ...
  * **📈 System Monitoring:** Get reports on CPU, RAM, disk usage, and system temperature.
  * **⚙️ Custom Scripts:** Define your own scripts (`.sh`, `.py`) in the configuration file to automate repetitive tasks.
  * **🤖 AI Integration (Gemini):** Use Google's AI to analyze logs or assist with complex tasks.
  * **🌐 Network Monitoring:** Perform pings, port scans (`nmap`), and check SSL certificate validity.
...
## 🛡️ Critical Security: "Sealing" Your Scripts with `seal_scripts.py`
...
#### How It Works
  ...
  * ✅ **If they match:** The script is safe and gets executed.
  * ❌ **If they DO NOT match:** The bot will refuse to run the script and send you an alert. This means the file has been altered\!
...
### 🌟 Características Principales
  ...
  * **🔌 Gestión de Servicios:** Inicia, detiene, reinicia y comprueba el estado de servicios del sistema (`systemd`) y contenedores Docker.
  * **🖥️ Ejecución de Comandos:** Ejecuta comandos de terminal de forma segura.
  * **🛡️ Seguridad Integrada:**
      * **Control de Acceso:** Solo los usuarios autorizados pueden interactuar con el bot.
      * **Verificación de Integridad de Scripts:** ¡NUEVO\! Usa hashes SHA256 para garantizar que los scripts no hayan sido manipulados.
      * **Prevención de Command Injection:** Uso de `subprocess` de forma segura.
      * **Gestión de Fail2Ban:** Desbanea IPs directamente desde el bot.
  * **📂 Gestión de Ficheros:** Sube y descarga ficheros desde y hacia el servidor.
  * **📈 Monitorización del Sistema:** Obtén informes sobre el uso de CPU, RAM, disco y la temperatura del sistema.
  * **⚙️ Scripts Personalizados:** Define tus propios scripts (`.sh`, `.py`) en el fichero de configuración para automatizar tareas repetitivas.
  * **🤖 Integración con IA (Gemini):** Utiliza la IA de Google para analizar logs o ayudarte con tareas complejas.
  * **🌐 Monitorización de Red:** Realiza pings, escaneos de puertos (`nmap`) y comprueba la validez de certificados SSL.
...
3.  **Configura el bot:**
      ...
      * Copia `configbot.json.example` a `configbot.json` y `users.json.example` a `users.json` y edítalos con tu información.
...
### 🛡️ Seguridad Crítica: "Sellar" tus Scripts con `seal_scripts.py`

Esta es la característica de seguridad más importante del bot. Para prevenir la ejecución de scripts que hayan sido modificados sin tu permiso (por ejemplo, por un atacante), el bot verifica la **integridad** de cada script antes de ejecutarlo.

#### ¿Cómo Funciona?

  * Cuando ejecutas el script `seal_scripts.py`, este calcula una "firma digital" (un hash SHA256) única para cada uno de tus scripts permitidos.
  ...
  * ✅ **Si coinciden:** El script es seguro y se ejecuta.
  * ❌ **Si NO coinciden:** El bot se negará a ejecutar el script y te enviará una alerta. ¡Esto significa que el fichero ha sido alterado\!

#### **Instrucciones de Uso (¡MUY IMPORTANTE\!)**

Cada vez que **modifiques** uno de tus scripts (`.sh` o `.py`) o **añadas uno nuevo** a `configbot.json`, DEBES volver a ejecutar el script de sellado.

1.  Modifica o crea tu fichero de script (ej. `~/telegram/scripts/sh/mi_nuevo_script.sh`).
2.  Añade el script a la sección `"scripts"` de tu `configbot.json`.
3.  Ejecuta el script de sellado:
    ```bash
    python3 seal_scripts.py
    ```
4.  El script actualizará automáticamente todos los hashes en `configbot.json`. ¡Tu bot ya confía en la nueva versión\!

-----

### 🛠️ Configuración

Toda la configuración se gestiona a través de los ficheros `configbot.json` y `users.json`.

  * **`users.json`**: Aquí defines quién es el Super Administrador y qué otros usuarios de Telegram están autorizados para usar el bot.

  * **`configbot.json`**:

      * **`telegram`**: Tu token y chat\_id.
      * **`gemini_api`**: Activa y configura la API de Gemini.
      * **`scripts`**: **(Estructura Unificada)** Define todos tus scripts aquí. Esta es la sección que `seal_scripts.py` utiliza.

    **Ejemplo de la sección `scripts`:**
...
Puedes ejecutar el bot directamente o como un servicio de `systemd` para que se inicie automáticamente.
  ...
  * **Como servicio (recomendado):**
    1.  Edita el fichero `telegram_bot.service` para que apunte a las rutas correctas de tu instalación.
    2.  Copia el fichero a systemd y actívalo:
        ```bash
        sudo cp telegram_bot.service /etc/systemd/system/
        sudo systemctl daemon-reload
        sudo systemctl enable telegram_bot.service
